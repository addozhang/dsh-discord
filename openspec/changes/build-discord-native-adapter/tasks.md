...
- [ ] 16.8 RED: update activity-row and notice tests to the icon-prefixed shapes (inline-snapshot style); GREEN: add `src/stream/icons.ts` as the single mapping module — state traffic-light set (🟡/✅/❌), category icons on the tool-label allowlist (⌨️📖✍️✏️🔍🗂️🌐, fallback 🧩), notice kinds (⚠️/💡/🛑/↪️/⏳) — and wire it through renderActivity, mainline failure copies, the unbound-channel notice, and stop/steer/queue acks. Sanitized text labels stay unchanged; assistant answers carry no icon prefix; text-only verbosity still omits rows. (stream-renderer "Fixed adapter-owned iconography")
