...
- [x] 16.5 Decide and implement the unbound-channel mention behavior required by session-control ("New task in an unbound channel" scenario): a minimal public bind-affordance notice (spec-as-written) versus silent ignore (current implementation); align the spec or the code accordingly. *(Implemented spec-as-written: an authorized mention in an unbound channel posts one public bind-affordance notice — authority-differentiated copy, zero DSH calls; composition dep `unboundNotice`, tested in compose.test.ts.)*
...
- [x] 16.8 Kimaki thread model (user-directed): DSH's session-title projection renames the Thread once per distinct title (the Kimaki rename behavior). Session-control spec gains the "Kimaki thread surface" requirement; design §4 records the model. Live-tested: rename verified in the test Guild.
- [x] 16.9 Correction after live test (Kimaki source verified, discord-bot.ts:1021): the opener is the ANCHORED source message (`message.startThread` = threads.create with message_id — Discord moves the user's task message into the Thread), and Kimaki DOES join the author ("add user to thread so it appears in their sidebar") — the webhook-mirror interpretation was wrong and is reverted. Adopted alongside: OneDay auto-archive. Author joins are logged on failure (runbook: silent failure is a bug).
- [x] 16.10 Auto-archive becomes a card setting: `threadAutoArchiveMinutes` joins the validated settings namespace (1 hour / 1 day / 3 days / 7 days, default one day) with a select control on the Discord settings card; new threads read it live, so changes apply without restart.
- [x] 16.11 Two real route defects found by the discord-digital-twin protocol audit (both live on real Discord too): recovery listed the nonexistent channel-scoped `GET /channels/:id/threads/active` (now the guild-scoped route), and anchored creation posted `message_id` in the channel-scoped body, which Discord ignores (now the documented `/channels/:id/messages/:mid/threads` route). Twin-E2E seams added: `DSH_DISCORD_GATEWAY_URL` and `DSH_DISCORD_REST_BASE` env overrides default to the production surfaces.
- [x] 16.12 Twin smoke coverage: `discord-digital-twin` (MIT, npm) installed as a dev dependency with a scoped react@19 override for spiceflow plus a plugin-rsc audit fix; `test/twin-smoke.test.ts` runs the REAL gateway client and REAL REST client against the twin over the same wire protocol while DSH stays a deterministic fake — mention → anchored thread → at-most-once prompt → thread continuation asserted on Discord-visible state only (Kimaki discipline).
- [x] 16.14 Admission-time typing (Kimaki pattern; user-reported dead zone between prompt submission and the first stream event): typing starts when the DSH queue snapshot turns non-empty (prompt enqueued) and stops when the queue drains with no open turn; typing lifecycle is recreated per turn so later turns type again. Stream-renderer "Typing lifecycle" requirement amended to cover the queued window.
- [x] 16.13 Interaction router extracted from the Host composition root into `src/features/interaction-router.ts` (autocomplete, slash commands, bind confirm/cancel components, approval-click fallback) with explicit injected deps — behavior preserved verbatim. Interaction smoke landed on the twin: bind (slash → deferred ack → confirm button → channel provisioning → durable binding), stop and steer (thread ownership + ephemeral ephemeral-flagged result copy) all driven through simulateSlashCommand/simulateButtonClick over the real wire.
- [x] 16.17 Phase 2 live-verified (2026-08-29): bindings/intents durable across restart — bind once, restart the host, reconcile sweep runs on READY and keeps the mapping, @bot works without re-binding; /project info intact. 15.10 step 7 (restart reconciliation) is thereby satisfied for the no-duplicate/no-resend half; in-flight uncertain prompts stay `unknown` by construction and are never silently resent.
- [x] 16.16 Activity rows redesigned (user-directed after live test; state marks removed): rows show the Host presentation view's title (a terminal call's command) beside one category icon (Shell 💻), no run-state marks — the earlier 🟡 was also permanently stuck because tool/result correlation reads `toolCallId`, not `callId` (fixed). Activity edits coalesce at 1s; the activity message is deleted exactly once at turn end. Stream-renderer requirements amended ("Minimal tool progress presentation" now sources row content from presentation views; "Fixed adapter-owned iconography" drops state marks).
- [x] 16.15 RED: update activity-row and notice tests to the icon-prefixed shapes (inline-snapshot style); GREEN: add `src/stream/icons.ts` as the single mapping module — exception-only state marks (🟡 running, ❌ failed; succeeded rows carry no state icon — the disappearing marker is the completion signal), category icons on the tool-label allowlist (⌨️📖✍️✏️🔍🗂️🌐, fallback 🧩), notice kinds (⚠️/💡/🛑/↪️/⏳) — and wire it through renderActivity, mainline failure copies, the unbound-channel notice, and stop/steer/queue acks. Sanitized text labels stay unchanged; assistant answers carry no icon prefix; text-only verbosity still omits rows. (stream-renderer "Fixed adapter-owned iconography"; DONE — `src/stream/icons.ts` is the single table, renderActivity rows are `🟡/❌ + category + label` with quiet succeeded rows edited in place, all ⛔/✅ notice copies migrated to the five kinds, assistant text untouched; twin smoke + 590 tests green)
