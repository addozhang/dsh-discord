...
- [x] 16.8 Kimaki thread model (user-directed): replace anchored creation + author-join with an unanchored Thread whose opener is the author's channel message mirrored once through a per-thread webhook rendering as the author (SUPPRESS_MENTIONS); crash-window recovery matches the deterministic task title; DSH's session-title projection renames the Thread once per distinct title (the Kimaki rename behavior). Session-control spec gains the "Kimaki thread surface" requirement; design §4 records the model. Live-tested: bind → mention → thread visible under its channel with the author-identity opener; title rename pending live verification.
