...
The adapter SHALL create a new task's Thread anchored to the source message (Discord moves the author's task message into the Thread as its first post), named after the deterministic task title with a configurable auto-archive duration (Discord-supported 1 hour, 1 day, 3 days, or 7 days; default one day). The adapter SHALL join the task author to the Thread — Discord sidebars list only threads the user has joined — and the join SHALL be best-effort: a failed join SHALL NOT fail the task. Crash-window recovery SHALL match the task's Thread deterministically by its anchored source-message id. When DSH publishes a model-generated Session title (derived from the user's first input), the adapter SHALL rename the Thread to it at most once per distinct title.

#### Scenario: Thread opens with the author's task
- **WHEN** an authorized user's mention creates a new Thread
- **THEN** the author's source message is the Thread's first post, and the Thread appears in the author's sidebar
