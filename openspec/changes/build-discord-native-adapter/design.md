...
The settings namespace holds non-secret values: required allowed Guild IDs; member user/role IDs; administrator user/role IDs; deny user/role IDs; a global Host-operator user-ID allowlist; default verbosity; bounded retry/timing values; and the task-thread auto-archive duration (1 hour / 1 day / 3 days / 7 days, default one day). Inside an allowed Guild, the Guild owner and members with Discord Administrator or Manage Guild are Workspace administrators by default. Workspace administrators also receive ordinary member access. Deny IDs override every level, including Host operator. Settings changes use revision fencing and apply live. The one bot token uses a fixed plugin-owned credential reference, is resolved for every connection attempt, and is never stored in settings or adapter state. Credential changes replace the active Gateway generation without restarting DSH.
