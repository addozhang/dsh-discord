...
The Thread follows Kimaki's surface model: it is created independently under the parent channel (unanchored), and the author's channel message is mirrored once into it as the opener through a reusable per-thread webhook that renders as the author (name + avatar, SUPPRESS_MENTIONS set) — the Thread reads as if the user had posted their task there themselves. The mirror is best-effort and never fails the task. The initial Thread name is the deterministic sanitized task title; crash-window recovery matches that title among the parent's active threads. When DSH later publishes its model-generated Session title (derived from the user's first input), the adapter renames the Thread once per distinct title — the Kimaki "renamed the channel" behavior — via the `session/projection` (`key: 'title'`) mux frame. The adapter does not add the author to thread members.
