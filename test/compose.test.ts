...
describe('composed adapter runtime', () => {
  it('starts the gateway and delivers an authorized mention into the DSH prompt flow', async () => {
    const { runtime, submit, gatewaySocketRef } = setup('token-abc', undefined)
    ...
    expect(submit.mock.calls[0]?.[0]).toEqual({
      requestId: 'discord:222222222222222222',
      sessionId: 'pending:ws-1',
  ...
  it('stays silent for unbound channels and unauthorized guilds', async () => {
    ...
    // Unbound channel: mention is deferred, no DSH call.
